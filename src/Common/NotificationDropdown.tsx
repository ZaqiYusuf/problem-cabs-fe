...
const notification: Notification[] = [
    { id: 1, type: "administrator", imageClassName: "size-10 rounded-md shrink-0 bg-slate-100", image: avatar3, boldName: "Admin", name: "notification", time: "4 sec",description:"Validity Periode", date: "Wednesday 03:42 PM" },
    { id: 2, type: "payment", imageClassName: "size-10 bg-yellow-100 rounded-md shrink-0", image: avatar3, boldName: "Finance", name: "notification", time: "15 min", description: "Payment Approval", date: "Wednesday 03:42 PM", },
                ...
                <Dropdown.Content placement="right-end" className="absolute z-50 ltr:text-left rtl:text-right bg-white rounded-md shadow-md !top-4 dropdown-menu min-w-[20rem] lg:min-w-[26rem] dark:bg-zink-600" aria-labelledby="notificationDropdown">
                    <div className="p-4">
                        ...
                        <ul className="flex flex-wrap w-full p-1 mb-2 text-sm font-medium text-center rounded-md filter-btns text-slate-500 bg-slate-100 nav-tabs dark:bg-zink-500 dark:text-zink-200">
                            ...
                            <li className={`group grow ${filter === "payment" ? "active" : ""}`} onClick={() => setFilter("payment")}>
                                <Link to="#" data-filter="payment" className="inline-block nav-link px-1.5 w-full py-1 text-xs transition-all duration-300 ease-linear rounded-md text-slate-500 border border-transparent group-[.active]:bg-white group-[.active]:text-custom-500 hover:text-custom-500 active:text-custom-500 dark:text-zink-200 dark:hover:text-custom-500 dark:group-[.active]:bg-zink-600 -mb-[1px]">Finance</Link>
                            </li>
                            <li className={`group grow ${filter === "administrator" ? "active" : ""}`} onClick={() => setFilter("administrator")}>
                                <Link to="#" data-filter="administrator" className="inline-block nav-link px-1.5 w-full py-1 text-xs transition-all duration-300 ease-linear rounded-md text-slate-500 border border-transparent group-[.active]:bg-white group-[.active]:text-custom-500 hover:text-custom-500 active:text-custom-500 dark:text-zink-200 dark:hover:text-custom-500 dark:group-[.active]:bg-zink-600 -mb-[1px]">Administrator</Link>
                            </li>
                                        ...
                                        {index === notification.length - 1 ?
                                            <div className="relative shrink-0">
                                                <div className="size-10 bg-pink-100 rounded-md">
                                                    <img src={avatar3} alt="" className="rounded-md" />
                                                </div>
