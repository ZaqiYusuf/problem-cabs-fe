...
import { PeriodeData } from "Common/data/dashboard";

const Table = () => {
  const [data, setData] = useState(PeriodeData);

  // Search Data
  const filterSearchData = (e: any) => {
    const search = e.target.value;
    const keysToSearch = ["name"];
    filterDataBySearch(PeriodeData, search, keysToSearch, setData);
  };
  ...
  const handleEdit = (periode: any) => {
    console.log("Edit periode: ", periode);
  };

  const handleDelete = (periode: any) => {
    console.log("Delete periode: ", periode);
  };
        ...
        <div className="card-body">
          <div className="grid items-center grid-cols-1 gap-3 mb-5 xl:grid-cols-12">
            <div className="xl:col-span-3">
              <h6 className="text-15">List Periodes</h6>
            </div>
